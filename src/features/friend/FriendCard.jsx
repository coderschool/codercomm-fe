...
function FriendCard({ friendship, request }) {
  ...
  if (friendship) {
    ...
    if (request.from === currentUser._id) {
      ...
      console.warn("FriendCard: Request object doesn't involve current user.", { request, currentUser });
    ...
    if (!displayUser) {
       console.warn("FriendCard: Missing sender/recipient data in request object.", { request });
       return (
          <Card className="p-3 border-destructive">
             <p className="text-xs text-destructive">Invalid request data (Missing user info)</p>
  ...
  return (
    <Card className={cn("p-3",
      ...
      {(actionContext === 'incoming_request' || actionContext === 'outgoing_request') && requestTimestamp && (
        <div className="ml-12 mt-1 flex items-center">
          <Clock className="w-3 h-3 mr-1 text-muted-foreground" />
          <p className="text-xs text-muted-foreground">
            Request {actionContext === 'outgoing_request' ? "sent" : "received"} {formatDistanceToNow(new Date(requestTimestamp), { addSuffix: true })}
          </p>
        </div>
      )}
    </Card>
...
FriendCard.propTypes = {
  ...
  request: PropTypes.shape({
    _id: PropTypes.string.isRequired,
    requester: PropTypes.shape({ // Expect sender details
        _id: PropTypes.string.isRequired,
        name: PropTypes.string,
        avatarUrl: PropTypes.string,
    }),
    recipient: PropTypes.shape({ // Expect recipient details
        _id: PropTypes.string.isRequired,
        name: PropTypes.string,
        avatarUrl: PropTypes.string,
    }),
